...
export default function ProductClientCircleAcrylic() {
  ...
  const sizeOptions = ["8x8", "10x10", "12x12", "16x16"];
  ...
  const generateMailPreviewImage = async () => {
    try {
      ...
      const baseScale = Math.max(
        exportSize / img.width,
        exportSize / img.height
      );
      ...
      const previewDims = frameDimensions[size] || { width: 200, height: 200 };
      const offsetScaleX = exportSize / previewDims.width;
      const offsetScaleY = exportSize / previewDims.height;

      const dx = (exportSize - drawWidth) / 2 + imageOffset.x * offsetScaleX;
      const dy = (exportSize - drawHeight) / 2 + imageOffset.y * offsetScaleY;
