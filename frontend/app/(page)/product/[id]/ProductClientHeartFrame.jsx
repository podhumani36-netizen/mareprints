...
// Heart SVG path — 300×280 bounding box
const HEART_PATH =
...
// 5 photo slots: 2 in top humps, 3 in middle body
const SLOT_CONFIG = [
  { id: 0, x: 18,  y: 18,  w: 105, h: 90, label: "Photo 1" },
  { id: 1, x: 177, y: 18,  w: 105, h: 90, label: "Photo 2" },
  { id: 2, x: 15,  y: 118, w: 82,  h: 72, label: "Photo 3" },
  { id: 3, x: 109, y: 118, w: 82,  h: 72, label: "Photo 4" },
  { id: 4, x: 203, y: 118, w: 82,  h: 72, label: "Photo 5" },
];

// evenodd path: heart shape minus rectangular slot holes
const SVG_FRAME_PATH =
  HEART_PATH +
  " M18,18 L123,18 L123,108 L18,108 Z" +
  " M177,18 L282,18 L282,108 L177,108 Z" +
  " M15,118 L97,118 L97,190 L15,190 Z" +
  " M109,118 L191,118 L191,190 L109,190 Z" +
  " M203,118 L285,118 L285,190 L203,190 Z";

function scalePath(pathStr, scale) {
...
export default function ProductClientHeartFrame() {
  ...
  const generateMailPreviewImage = async () => {
    try {
      ...
      // Draw heart frame overlay
      const scaledPath = scalePath(SVG_FRAME_PATH, SCALE);
      const p = new Path2D(scaledPath);
      ctx.fillStyle = frameColor;
      ctx.fill(p, "evenodd");

      // Heart border
      const heartP = new Path2D(scalePath(HEART_PATH, SCALE));
  ...
  const renderHeartPreview = (useWall = false) => (
    ...
    >
      ...
      {/* Heart frame container — clips everything to heart shape */}
      <div
        style={{
          position: "absolute",
          left: "50%",
      ...
      >
        ...
        {SLOT_CONFIG.map((slot) => (
          ...
          >
            {images[slot.id] ? (
              ...
              >
                <i className="bi bi-plus-circle" style={{ fontSize: 18 }} />
                <span style={{ fontSize: 9, fontWeight: 600, textAlign: "center" }}>
                  {slot.label}
                </span>
              </div>
            )}
        ...
        {/* SVG frame overlay — heart material with slot holes */}
        <svg
          width="300"
          height="280"
          viewBox="0 0 300 280"
        ...
        >
          <path d={SVG_FRAME_PATH} fill={frameColor} fillRule="evenodd" />
          {/* Gold border */}
          <path d={HEART_PATH} fill="none" stroke="#c9952a" strokeWidth="2.5" />
          {/* Decorative "Family" text between photo rows */}
          <text
            x="150"
            y="114"
            textAnchor="middle"
      ...
      >
        {[
          ...
          `${images.filter(Boolean).length}/${SLOT_CONFIG.length} photos`,
        ].map((label) => (
          <span
            key={label}
            style={{
            ...
            <div key={step} className={styles.stepItem}>
              ...
              >
                ...
                <span className={styles.stepLabel}>
                  {step === 1 ? "Upload Photos" : "Customize & Payment"}
                </span>
              </button>
              {step < 2 && (
                <div
          ...
          <div className="col-sm-12 col-lg-8">
            <div className={styles.uploadCard}>
              ...
              >
                ...
                >
                  {isDragging
                    ? `Drop to upload into ${SLOT_CONFIG[activeSlot].label}`
                    : "Click a slot in the heart to upload a photo"}
                </p>
            ...
            {/* Slot quick-select buttons */}
            <div
              className={`${styles.uploadCard} mt-4`}
            ...
            >
              ...
              >
                <i
                  ...
                  style={{ color: "#e11d48" }}
                />
                Heart Collage Frame — select a slot then browse or drag & drop
              </p>
                ...
                {SLOT_CONFIG.map((slot) => (
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 600,
                      cursor: "pointer",
                      background: images[slot.id]
                        ? "#dcfce7"
                        : activeSlot === slot.id
                        ? "#0f172a"
                        : "#f1f5f9",
                      color: images[slot.id]
                        ? "#16a34a"
                        : activeSlot === slot.id
                        ? "#fff"
                        : "#475569",
          ...
          <div className="col-sm-12 col-lg-4">
            <div className={styles.guideCard}>
              ...
              <ul className={styles.guideList}>
                <li>
                  <i className="bi bi-check-circle-fill" /> Click any slot in
                  the heart preview
                </li>
                <li>
                  <i className="bi bi-check-circle-fill" /> Each slot holds one
                  photo
                </li>
                <li>
                ...
                <li>
                  <i className="bi bi-check-circle-fill" /> Square images fill
                  slots best
                </li>
                <li className={styles.warning}>
                  <i className="bi bi-exclamation-triangle-fill" /> At least 1
                  photo required
                </li>
              </ul>
              ...
              >
                ...
                <div style={{ fontSize: 13, color: "#92400e", marginBottom: 8 }}>
                  {images.filter(Boolean).length} of {SLOT_CONFIG.length} slots
                  filled
                </div>
                <div
                  style={{
                ...
                >
                  <div
                    style={{
                      ...
                      borderRadius: 99,
                      width: `${
                        (images.filter(Boolean).length / SLOT_CONFIG.length) *
                        100
                      }%`,
              ...
              <div style={sectionCardStyle}>
                <div className="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-3">
                  <div>
                    ...
                    >
                      Review your heart collage frame
                    </p>
                  </div>
                  <button
                    type="button"
                  ...
                  {!isPaymentReady ? (
                    ...
                    <RazorpayPayment
                      ...
                      productDetails={{
                        ...
                        orderId,
                        frameColor,
                        photosCount: images.filter(Boolean).length,
                      }}
          ...
          <div className="modal-content" style={{ borderRadius: 24 }}>
            <div className="modal-body text-center p-5">
              ...
              <p style={{ color: "#64748b", marginBottom: 20 }}>
                Thank you for your order. Your heart collage frame is being
                prepared with love.
              </p>
              <button
                type="button"
